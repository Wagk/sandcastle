/*!************************************************************
  ...
  Usage	:
    ```
      ...
      struct B : Reflectable
      {
        ...
        bool m_bool[3];
        std::vector m_vec; //not reflectable, so we use string operator
      };

      Propertydata data; //lets say it's already loaded with the string goods
  ...
  Note	:
    Don't forget this struct ACTUALLY CONTAINS STRING DATA
    to manipulate
...
namespace Reflection
{
  ...
  //helper types
  template<typename T>
  using MirroredType = typename std::enable_if<std::is_base_of<Reflectable, T>::value == true, T>::type;
  template<typename T>
  using NonMirroredType = typename std::enable_if<std::is_base_of<Reflectable, T>::value == false, T>::type;
